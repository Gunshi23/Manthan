...
Manthan is designed as a dynamic, real-time command center coordinating multi-agent generative marketing pipelines with analytics ledger synchronization, client credentials updates, and a voice-uplink telemetry interface.
...
Manthan assigns growth activities to five distinct AI Agent roles (configured in [geminiService.ts](file:///c:/Users/sachin/OneDrive/Desktop/ORBIT1/server/services/geminiService.ts)):
- **Drishti (Audience Intelligence)**: Segments cohorts (`Loyalists`, `Slipping Away`, `High-Value Inactive`, `New Signups`) to pinpoint high-potential targets.
- **Pragya (Recovery Analysis)**: Scans for cart drop-offs, leakage points, and computes metrics (recoverable revenue, recovery confidence).
- **Khoj (Predictive ROI)**: Generates quantitative revenue forecast models and confidence intervals.
- **Rachna (Creative Campaign Copywriter)**: Compiles personalized messaging copy tailored per delivery channel.
- **Saarthi (Operations / Dispatch Routing)**: Chooses optimal communication channels and validates the dispatch gateway routing parameters.
...
* [MissionControl.tsx](file:///c:/Users/sachin/OneDrive/Desktop/ORBIT1/src/pages/MissionControl.tsx) - Central operational command showing KPI cards, real-time agent execution visual pipelines, Pragya Opportunity charts, and live agent log tracking.
